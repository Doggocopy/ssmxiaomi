		...
		<div class="myorder">
			<table border="0px" width="1226" style="text-align: center;">
				<tr>
					...
					<td width="100" height="70;"><fmt:formatDate value="${order.odate }" pattern="yyyy-MM-dd"></fmt:formatDate> </td>
			...
			<div id="detailDiv_${order.oid }" style="display: none;">
				<table border="0px" width="1226" style="text-align: center;">
					...
					<tr>
						<td colspan="3">收货地址：${order.addressobj.address }</td>
						<td colspan="2">收货人：${order.addressobj.cnee }</td>
						<td colspan="3">电话：${order.addressobj.phone }</td>
					</tr>
					<tr>
						<td colspan="8"><hr></td>
					</tr>
					<c:forEach var="detail" items="${order.detailList}">
						<tr style="font-size: 18px;">
							<td width="100"><img alt="" src="image_big/${detail.prod.pImage }" height="100px;"></td>
							<td width="100">${detail.prod.pName }</td>
							<td width="100">${detail.prod.pPrice }元</td>
							<td width="100">${detail.pnumber }件</td>
							<td width="250">${detail.prod.pContent }</td>
							<td width="250">小计：${detail.ptotal}元</td>
							<td width="100">
								<c:if test="${order.status eq '已发货' }">
									<a href="prod.do?flag=comment&pid=${detail.prod.pId }"
